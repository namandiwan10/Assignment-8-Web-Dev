...
exports.create = (req, res) => {
    ...
    if (!req.body) {
        res = res.status(400).send({ message: "Content can not be empty!" });
        res.message = "Content can not be empty!";
        return;
    }
    ...
    if (!IsEmail(user.email)) {
        res.status(400).send({
            message: "Wrong Email format. Please retry entering northeastern domain"
        });
        return;
    }
...
exports.update = (req, res) => {
    ...
    if (!req.body) {
        return res
            .status(400)
            .send({ message: "Data which needs to be updated cannot be empty" })
    }
    ...
    Userdb.findOneAndUpdate(inputEmail, req.body)
        .then(data => {
            if (!data) {
                res.status(404).send({ message: `Cannot Delete with id ${inputEmail}. InputEmail is wrong` })
            } else {
                res.send({
